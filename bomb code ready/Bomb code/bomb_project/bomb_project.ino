...
char password[7]; // number of characters in our password
...
byte rowPins[ROWS] = {A2, A1, 5, A0}; //connect to the row pinouts of the keypad
  ...
  if (key2 == '*')
    {
      ...
      if (currentLength == 7) 
        {
          if (entered[0] == password[0] && entered[1] == password[1] && entered[2] == password[2] && entered[3] == password[3] && entered[4] == password[4] && entered[5] == password[5] && entered[6] == password[6])
            {
              lcd.noCursor();
              lcd.clear();
              lcd.home();
